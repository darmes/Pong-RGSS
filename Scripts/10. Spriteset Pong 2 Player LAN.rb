...
class Spriteset_Pong_Two_LAN
  ...
  def update
    ...
    if @ball.x <= @player_2.ex and
       (@player_2.y...@player_2.ey).include?(@ball.y) and
			 @ball.x_vector < 0
				 @ball.paddle_bounce
		end
		# if ball is touching player_2 paddle and moving towards it
		if @ball.ex >= @player_1.x and
		   (@player_1.y...@player_1.ey).include?(@ball.y) and
		   @ball.x_vector > 0
			   @ball.paddle_bounce
		end
